...
if (isset($_GET['view'])) {
    ...
    header("location: view.liaising.php");
}
if (isset($_GET['restore'])) {
    $property = new ManagePropertyContr();
    $result = $property->approveProperty($_GET['restore'], 0);

    if ($result) {
        $msg = "Property restored";
    } else {
        $msg2 = "Couldn't restore property";
    }
}

if (isset($_GET['delete'])) {
    $property = new ManagePropertyContr();
    $result = $property->deleteProperty($_GET['delete']);

    if ($result) {
        $msg = "Property deleted";
    } else {
        $msg2 = "Couldn't deleted property";
    }
}
?>
<!DOCTYPE html>
        ...
        <div class="table-responsive">
            <table class="table table-striped" id="example" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    $row = $property->viewPropertyAdmin(4, 4, 4, 4, 4);
                    $index = 1;
                    foreach ($row as $rw) {
                        ...
                        <td><button class='p-0' data-bs-toggle='modal' data-bs-target='#image' style='border-radius: 5px;'><img src='../uploads/" . $rw['image1'] . "' alt='image' width='60' height='40'></button></td>"; ?>
                        <td>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="logoutModalLabel">Property Image</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <img src="../uploads/<?php echo $rw['image1']; ?>" width="100%" alt="national ID image">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="btn btn-sm m-0 p-0">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item text-primary" href="property_archive.liaising.php?view=<?php echo $rw['prop_id']; ?>"><span class="fas fa-eye mr-2"></span> View</a>
                                    <a class=" dropdown-item text-success" href="property_archive.liaising.php?restore=<?php echo $rw['prop_id']; ?>" onclick="return confirm('are you sure you want to restore?')"><span class="fa fa-recycle mr-2"></span> Restore</a>
                                    <a class="dropdown-item text-danger" href="property_archive.liaising.php?delete=<?php echo $rw['prop_id']; ?>" onclick="return confirm('are you sure you want to delete?')"><span class="fas fa-trash-alt mr-2"></span> Delete</a>
                                </div>
                            </div>
