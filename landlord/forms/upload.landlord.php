...
if (isset($_POST['submit'])) {
    ...
    $st = $upload->submitProperty($user_id, $type, $name, $price, $period, $duration, $quantity, $land, $district, $area, $descript,  $_FILES['images']['name'][0],  $_FILES['images']['name'][1],  $_FILES['images']['name'][2],  $_FILES['images']['name'][3]);

    //checking if the data has been uploaded -------------------------------------------------------------------------->
    if ($st) {
        $_SESSION['coord_type'] = $type;
        $_SESSION['coord_dist'] = $district;
        $_SESSION['coord_area'] = $area;
        header("location: geolocation.landlord.php");
    } else {

        $msg2 = "Error, failure submitting property!";
    }
}
